...
FILE_NAME_PRESUPUESTO = './tensor_data/query_series.csv'
...
def generate_cvs_presupuesto_detalle_servicio(FILE_NAME=FILE_NAME_PRESUPUESTO):
    ...
    # convert list datetime to string
    data = [(i['presupuesto_servicio__fecha_servicio'].strftime('%Y-%m-%d'), i['servicio__id_servicio']) for i in data]
...
def split_train_test_set(reframed):
...
def show_results_accuracy(instance, results, y_val, x_val):
    ...
    plt.title('Validando la efectividad del modelo de Tensorflow (valores-verdes) (resultados-rojos)')
    # plt.show()
...
def comparate_results(instance, scaler, results, y_val):
    ...
    plt.xlabel('Dias')
    plt.title('Comparacion de resultados modelo entrenado de Tensorflow model (real-azul) (predicciones-naranja)')
    plt.savefig(figure, format='png')
    # plt.show()
...
def show_predicted_image(instance, predicted):
    ...
    plt.ylabel('Id de Servicios en la Base de Datos')
    plt.xlabel('Siguientes Dias')
    plt.title('Probable services ids en los siguientes dias')
    # plt.show()
    plt.savefig(figure, format='png')
    content_file = ImageFile(figure)
...
def main_generate_predictible(model_instance, predict_days=7):
    ...
    """
    cvs_generated = generate_cvs_presupuesto_detalle_servicio()
    ...
    x_train, y_train, x_val, y_val = split_train_test_set(reframed)
    ...
    # Predict next days
    predicted = predict_next_days(model_trained, scaler, pre_values_x, predict_days) # default 7 days
    ...
    convert_list_predicted = [int(round(x)) for x in list_predicted]
